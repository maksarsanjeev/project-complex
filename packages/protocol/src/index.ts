...
export interface SessionState {
  ...
  graph: GraphDoc
  /**
   * Снимки модели ИМЕННО ЭТОЙ сессии — по одному на движок.
   *
   * Проект бывает открыт сразу в SketchUp, Rhino и Blender, поэтому снимков
   * несколько, и в дереве они становятся тремя ветками верхнего уровня.
   * Привязка к сессии обязательна: иначе новый проект показывает модель
   * предыдущего, и «проект» отличается от «проекта» только перепиской.
   */
...
/**
 * Чем узел является НА ЯЗЫКЕ СВОЕГО ДВИЖКА.
 *
 * Три движка организуют модель по-разному, и сводить их к одному слову нельзя:
 * тег SketchUp ничего не содержит, слой Rhino содержит и вкладывается,
 * коллекция Blender содержит и допускает объект сразу в нескольких. Поэтому
...
export interface SceneNode {
   ...
   *
   * У Blender объект бывает сразу в нескольких коллекциях, у Rhino — сразу в
   * нескольких группах. Дерево такого не выражает: у узла ровно один родитель.
   * Поэтому первый становится родителем, а остальные попадают сюда и
   * показываются пометками — так мы не ломаем дерево и не врём о структуре.
   */
  memberships?: string[]
  /** Движок, которому принадлежит узел. У корня движка — он сам. */
  engine?: EngineId
}
...
export interface ModelSnapshot {
  ...
  /**
   * Сцена целиком в glb, если движок отдаёт её файлом.
   *
   * Так устроен Blender: у него есть штатный экспорт, и собирать
   * треугольники поштучно, как у Rhino, незачем. Вьюпорт читает glb с тех
   * пор, как появилось перетаскивание файлов, — поэтому геометрия
   * показывается сразу, а дерево наполняется отдельно, из списка объектов.
   */
  glb?: string
  takenAt: Timestamp
}
