...
export function handleAgentSocket(socket: WebSocket): void {
  ...
  socket.on('message', (raw) => {
    ...
    if (!agent) {
      ...
      /*
        Одна машина — одно соединение. Прежнее закрываем сами.

        Причина, по которой это стоит трёх повторов в логах: агент
        переподключается сам — после перезапуска, обрыва сети, сна машины, — а
        старый сокет со стороны сервера умирает не сразу и не всегда. Пока он
        числится живым, движки с него остаются в инвентаре, вызовы уходят
        именно в него и висят до таймаута. Пульс такое соединение в конце
        концов уберёт, но «в конце концов» — это десятки секунд, за которые
        человек успевает решить, что всё зависло.

        Ждать нечего: новое соединение с той же машины означает, что старое
        больше не нужно, кем бы оно ни казалось.
      */
      for (const [id, old] of agents) {
        if (old.machine !== frame.machine) continue
        agents.delete(id)
        for (const waiting of old.pending.values()) {
          clearTimeout(waiting.timer)
          waiting.reject(new Error('машина переподключилась, вызов отменён'))
        }
        old.pending.clear()
        old.socket.terminate()
        log(`${frame.machine}: закрыл прежнее соединение — машина подключилась заново`)
      }
