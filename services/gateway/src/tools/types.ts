...
export interface ToolDef {
  ...
  command: string
  /**
   * Команда, которую надо выполнить ПЕРЕД основной.
   *
   * Нужна там, где одно осмысленное действие человека распадается на две
   * команды моста. Пример: «посмотреть» — это навести камеру и снять кадр.
   * Отдельными инструментами это стоило бы модели двух кругов вместо одного,
   * а на шуруповёрте четыре взгляда съели восемь кругов из двадцати.
   */
  preCommand?: { command: string; params?: Record<string, unknown> }
  /**
